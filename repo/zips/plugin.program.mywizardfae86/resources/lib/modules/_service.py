...
class Startup:
    ...
    def check_updates(self):
           if current_build == 'No Build Installed':
               nobuild = dialog.yesnocustom(addon_name, 'Actualmente no hay ninguna construcción instalada.\n¿Te gustaría instalar una ahora?', 'Recordar más tarde')
               if nobuild == 1:
    ...
    def save_menu(self):
        save_items = []
        choices = ["Datos de Trakt & Debrid", "Claves de API de YouTube", "Favoritos", "Configuración avanzada", "Fuentes"]
        save_select = dialog.multiselect(addon_name + ' - ' + local_string(30052),choices, preselect=[])  # Seleccionar elementos para guardar
        if save_select == None:
            return
        else:
        ...
        if 'Datos de Trakt & Debrid' in save_items:
    ...
    def run_startup(self):
        ...
        if setting('firstrun') == 'true':
            ...
            xbmc.sleep(1000)
            self.notify_check()
            xbmc.sleep(3000)      #Retraso en la notificación de actualización de la construcción
            self.check_updates()

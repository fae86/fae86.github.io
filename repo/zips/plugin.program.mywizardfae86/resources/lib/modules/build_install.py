...
def build_install(name, name2, version, url):
    # Preparado para instalar, Cancelar, Continuar
    ...
    dialog.ok(addon_name, local_string(30036))  # Instalación completa
    os._exit(1)
...
def download_build(name, url):
    ...
    if 'dropbox' in url:
        if not xbmc.getCondVisibility('System.HasAddon(script.module.requests)'):
            ...
            dialog.ok(color2(name), color2(local_string(30033)))  # Instalando Requests
            return
...
def extract_build():
    if os.path.exists(zippath):
        dp.create(addon_name, local_string(30034))  # Extrayendo archivos
        counter = 1
        with ZipFile(zippath, 'r') as z:
            ...
            for file in files:
                ...
                try:
                    ...
                    xbmc.log(f'Error extrayendo {filename} - {e}', xbmc.LOGINFO)
                dp.update(progress_percentage, f'{local_string(30034)}...\n{progress_percentage}%\n{filename}')
                counter += 1
        dp.update(100, local_string(30035))  # Extracción completada
        xbmc.sleep(500)
        dp.close()
        os.unlink(zippath)
            ...
            if addon_xml.exists():
              ...
              if 'kodi.binary' in _xml:
                  ...
                  try:
                      ...
                      xbmc.log(f'No se pudo eliminar el binario {folder} - {e}')
...
def enable_wizard():
    try:
        ...
        xbmc.log('Hubo un error al escribir en la base de datos - %s' %e, xbmc.LOGINFO)
        return
    finally:
        try:
            if con:
